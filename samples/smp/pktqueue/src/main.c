...
u8_t headers[QUEUE_NUM][SIZE_OF_QUEUE][SIZE_OF_HEADER];
...
void init_datagram_queue(struct phdr_desc_queue *queue, int queue_num)
{
	...
	for (int i = 0; i < SIZE_OF_QUEUE; i++) {
		...
		descriptors[queue_num][i].ptr = (u8_t *)&headers[queue_num][i];
		/* Fill packet header with random values */
		for (int j = 0; j < SIZE_OF_HEADER; j++) {
			/* leave crc field zeroed */
			if (j < 10 || j > 11)
				descriptors[queue_num][i].ptr[j] = (u8_t)sys_rand32_get();
			else
