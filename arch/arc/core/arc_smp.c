 ...
 *
 * @param old, current thread pointer will be saved in old
 *
 * @return new thread pointer
 *
 * @details in isr, two values are required, the new thread pointer and
 * old thread pointer, if new thread pointer is not null, a thread switch
 * is required, old thread pointer will be used to save context; if new
 * thread pointer is null, no thread switch
 */
struct k_thread *z_arc_smp_switch_in_isr(struct k_thread **old)
{
